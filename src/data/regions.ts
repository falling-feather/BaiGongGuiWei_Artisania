 ...
 */
import type { RegionDef, RegionEndingDef, SubregionDef } from '../engine/types';
...
const REGION_SUBREGIONS: Record<string, SubregionDef[]> = {
  ...
  bashu: [
    ...
    { id: 'bashu-linqiong-iron', name: '临邛铁炉', role: '山城冶炉', blurb: '矿料、炭火与盐井旧道交织，是巴蜀铁器的试炼场。', traits: ['冶铁', '炉火', '山城'] },
    { id: 'bashu-tea-horse', name: '茶马驿', role: '山路驿站', blurb: '茶篓、马帮与栈道在此换脚，通向雪域与云贵。', traits: ['茶马', '驿站', '远行'] },
  ],
  lingnan: [
    ...
    { id: 'lingnan-gambiered-yard', name: '莨绸晒场', role: '染整晒场', blurb: '薯莨汁、河泥与日光轮番上场，绸面在晒棚间渐沉。', traits: ['染整', '日晒', '河泥'] },
    { id: 'lingnan-duan-stone', name: '端石坑口', role: '砚石山坑', blurb: '石坑潮气与砚工灯火相接，挑石要先听声再下凿。', traits: ['砚石', '采石', '文房'] },
  ],
  qiandian: [
    ...
    { id: 'qiandian-dongchuan-copper', name: '东川铜矿', role: '铜山矿口', blurb: '铜脉、矿灯与炼炉沿山势铺开，粗粝却满是生计。', traits: ['铜矿', '炼铜', '山路'] },
  ],
  jingchu: [
    ...
    { id: 'jingchu-chu-lacquer', name: '楚漆坊', role: '漆器工坊', blurb: '朱黑漆色在灯下铺开，纹样从古器与水泽里长出。', traits: ['漆作', '彩绘', '古纹'] },
    { id: 'jingchu-xiang-embroidery', name: '湘绣楼', role: '绣坊楼阁', blurb: '绷架、丝线与兽毛针法并列，针脚里藏着湖湘性情。', traits: ['刺绣', '丝线', '楼坊'] },
  ],
  ganpo: [
    ...
    { id: 'ganpo-river-wood', name: '河运柴场', role: '柴运码头', blurb: '窑柴在河边堆成山，船价与窑期一同牵动瓷镇。', traits: ['河运', '柴料', '议价'] },
  ],
  huizhou: [
    ...
    { id: 'huizhou-she-stone', name: '歙石山坑', role: '砚石坑口', blurb: '石脉隐在溪谷深处，砚师凭声色判断一块石的前途。', traits: ['砚石', '文房', '山坑'] },
    { id: 'huizhou-merchant-hall', name: '徽商会馆', role: '商帮会馆', blurb: '账册、路引与押货文书堆满案头，信用在这里有了重量。', traits: ['商帮', '会馆', '信用'] },
  ],
  jingji: [
    ...
    { id: 'jingji-official-gate', name: '官署门房', role: '采办门房', blurb: '门房、名帖与采办许可相互扣合，名声也是通行凭证。', traits: ['官署', '采办', '名声'] },
  ],
  sanjin: [
    ...
    { id: 'sanjin-lacquer-yard', name: '推光漆院', role: '漆器院落', blurb: '掌温、漆面与描金细活在小院里慢慢磨出光。', traits: ['漆器', '推光', '院落'] },
    { id: 'sanjin-vinegar-yard', name: '清徐醋坊', role: '发酵作坊', blurb: '缸口酸香、夏伏冬捞，日常饮食也有耐心的工艺。', traits: ['醋坊', '发酵', '民生'] },
  ],
  xueyu: [
    ...
    { id: 'xueyu-pigment-valley', name: '颜料矿谷', role: '矿彩山谷', blurb: '彩石在雪线下闪光，画师与采集者都在等一块好色。', traits: ['矿彩', '采集', '雪线'] },
    { id: 'xueyu-silver-tent', name: '银器帐房', role: '高原银作', blurb: '银片、松石与经幡影子铺在帐中，纹样带着祝祷。', traits: ['银器', '帐房', '镶嵌'] },
  ],
  xiyu: [
    ...
    { id: 'xiyu-caravan-post', name: '驼队驿站', role: '商队驿站', blurb: '水袋、驼铃与护商契约决定下一段沙路能不能走稳。', traits: ['驼队', '驿站', '护商'] },
    { id: 'xiyu-atlas-loom', name: '艾德莱斯织坊', role: '丝绸织坊', blurb: '扎经染色后的丝线像云霞，织机声在绿洲边跳动。', traits: ['织坊', '染色', '丝路'] },
  ],
};
...
const BASE_REGIONS: Array<Omit<RegionDef, 'subregions'>> = [
  {
    ...
    industries: ['build-indigo', 'split-bamboo', 'sericulture', 'weave-brocade', 'smelt-iron', 'forge-sword', 'mine-kaolin', 'pick-tea'],
    localResources: ['indigoPlant', 'bambooRaw', 'cocoonSilk', 'brineSalt', 'kaolin', 'ironOre', 'coal', 'teaLeaf'],
    signatureCrafts: ['indigo-dyeing', 'bamboo-weaving', 'celadon', 'longquan-sword', 'kesi', 'oilpaper-umbrella'],
    neighbors: ['huizhou', 'ganpo', 'jingji'],
    traits: ['富庶', '重商', '商路枢纽'],
    startUnlocked: true,
  },
  {
    ...
    neighbors: ['qiandian', 'jingchu', 'xueyu'],
  ...
  },
  {
    ...
    neighbors: ['bashu', 'qiandian', 'ganpo'],
    traits: ['铜铁矿冶', '江湖纵横'],
    startUnlocked: false,
  },
  {
    ...
    neighbors: ['jiangnan', 'huizhou', 'jingchu'],
    traits: ['瓷土与制瓷中心', '分工极细'],
    startUnlocked: false,
  },
  {
...
const REGION_ENDINGS: Record<string, RegionEndingDef> = {
  jiangnan: {
    trusted: '水市与书院开始认得百工院的招牌，竹器、蓝布和题跋作品能在江南日常里留下姓名。',
    honored: '秦淮灯下、太湖织埠与临安雨市相互应和，江南不只把玩家当外来掌柜，而把百工院当成能托付订单的邻里。',
    pillar: '江南诸坊把玩家推为坐地匠首，灯市、文房、织造和家园来访连成稳定的本地人情网。',
  },
  bashu: {
    trusted: '竹海、铁炉和茶马驿记住了百工院，巴蜀材料开始稳定支撑江南伞棚、织作和远行训练。',
    honored: '锦里工巷与青神竹海愿意把慢工交给玩家，山路订单不再只是买料，而成为跨地区供应链的筋骨。',
    pillar: '巴蜀把玩家视作能压住山路风险的人，竹、铁、茶和蜀锦在百工院形成可靠的西南支点。',
  },
  lingnan: {
    trusted: '珠江商港愿意给玩家试船期，外销、冶铸和莨绸晒场开始接进百工院账本。',
    honored: '岭南货栈把玩家列入熟客，海贸订单、天气晒制和端砚文房线不再彼此孤立。',
    pillar: '岭南船期因玩家而稳，港口、晒场、冶坊和砚坑共同撑起面向远海的百工院声名。',
  },
  qiandian: {
    trusted: '苗寨银巷与茶马驿道开始放低戒心，银铜、靛染和茶路为百工院开出第一段云贵深山路。',
    honored: '黔滇把玩家当成懂礼也懂路的人，银饰纹样、铜料与茶马补给在订单里彼此扣合。',
    pillar: '云贵山寨愿意把最讲规矩的银染交给玩家，百工院由此拥有礼俗、贵金属和远行交通的高地。',
  },
  jingchu: {
    trusted: '江湖渡市愿意让玩家押第一批水路货，漆作、矿冶和绣楼开始成为中游缓冲带。',
    honored: '荆楚水路把玩家当成懂风险的人，楚漆修复、铜铁矿冶和渡船商账形成稳定的中段网络。',
    pillar: '长江中游因玩家而成活水枢纽，矿料、漆器和绣作都能借百工院避开贪快的险滩。',
  },
  ganpo: {
    trusted: '窑火瓷镇开始让玩家看窑期，瓷土、柴路和湖船价进入百工院的日常盘算。',
    honored: '赣鄱窑口愿意把开窑风险交给玩家分担，龙泉、徽商和瓷镇之间有了可循环的柴土账。',
    pillar: '赣鄱把玩家视作能稳住窑火的人，瓷土、窑柴和湖路成为百工院高温工艺的主梁。',
  },
  huizhou: {
    trusted: '纸谷、墨坊和会馆开始认得玩家，徽州文房材料能接上江南题跋与百工院陈列。',
    honored: '徽州愿意把纸墨信用押给玩家，马头墙下的账本、纸槽和砚石坑口形成文房经济线。',
    pillar: '徽州把玩家列入可靠商帮，纸、墨、砚与题跋名声共同把百工院托成雅器根基。',
  },
  jingji: {
    trusted: '都门市口开始给玩家递采办话头，宫造订单不再只是传闻，而成为可试探的中期门槛。',
    honored: '京畿门房与宫造大院愿意看玩家名帖，规矩、商誉和重器材料在百工院账上并行。',
    pillar: '京畿把玩家列入能担保的工名，宫造重器与漕运采办为百工院打开最苛刻也最显眼的门。',
  },
  sanjin: {
    trusted: '票号古街开始给玩家一笔可控信用，煤铁、漆器和押金账不再只是风险。',
    honored: '三晋票号愿意替玩家压一段长账，信用订单、断供补救和北地工艺形成稳路底盘。',
    pillar: '三晋把玩家当成守信掌柜，票号、煤铁和推光漆院共同让远距离订单有了可承受的账期。',
  },
  xueyu: {
    trusted: '雪山驿口开始承认玩家的敬畏，矿彩、银器和唐卡材料不再只是稀有货名。',
    honored: '雪域愿意让玩家参与高原补给，心境、颜料和护商风险一起成为百工院后期试炼。',
    pillar: '雪域把玩家视为懂规矩的远行人，唐卡画院、颜料矿谷和驿口补给共同守住高压线路。',
  },
  xiyu: {
    trusted: '绿洲巴扎开始与玩家换熟价，美玉、驼队和丝路织坊有了初步信用。',
    honored: '西域愿意把稀有料和沙路补给交给玩家议价，巴扎不再只是商店，而是远行信用的终端。',
    pillar: '西域把玩家当成能守住来路的人，玉场、驼队和艾德莱斯织坊共同撑起百工院的远方名声。',
  },
};
...
export const REGIONS: RegionDef[] = BASE_REGIONS.map((region) => ({
  ...region,
  ending: REGION_ENDINGS[region.id],
